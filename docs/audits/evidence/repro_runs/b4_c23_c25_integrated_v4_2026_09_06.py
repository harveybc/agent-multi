"""INTEGRATED V4 (order C23-C25): all 12 cells through the FULL
...
OUT = STATE / "b4_integrated_v4_20260906"
...
with orch.GlobalLock(OUT):
    for cid in ledger_mod.EXPECTED_CELLS:
        ...
        detail = {
            ...
            "artifact_class": "INTEGRATED_PROOF_FROZEN_GENESIS_V4",
...
assert facts["n"] == 12
print(json.dumps({"integrated_v4": "COMPLETE",
...
for src, dst in (
        (OUT / cid / "B4_CELL_TERMINAL.json",
         EVID / "INTEGRATED_V4_TERMINAL_SAMPLE.json"),
        (OUT / cid / f"CLAIM_{b4a.CAMPAIGN_GENERATION}.json",
         EVID / "INTEGRATED_V4_CLAIM_SAMPLE.json"),
        (OUT / cid / f"SEAL_INTENT_{att}.json",
         EVID / "INTEGRATED_V4_SEAL_INTENT_SAMPLE.json"),
        (OUT / cid / f"SEAL_COMPLETE_{att}.json",
         EVID / "INTEGRATED_V4_SEAL_COMPLETE_SAMPLE.json")):
    dst.write_text(_san(src.read_text()))
(EVID / "INTEGRATED_V4_SUMMARY.json").write_text(json.dumps({
    "schema": "agent_multi.b4_integrated_v4_summary.v1",
    "order": "C23-C25 (final authority order 2026-09-06)",
    "path_proven": [
