"""INTEGRATED V5 (order C27-C28): all 12 cells through the FULL
...
OUT = STATE / "b4_integrated_v5_20260906"
...
with orch.GlobalLock(OUT):
    for cid in ledger_mod.EXPECTED_CELLS:
        ...
        detail = {
            ...
            "artifact_class": "INTEGRATED_PROOF_FROZEN_GENESIS_V5",
...
assert facts["n"] == 12
print(json.dumps({"integrated_v5": "COMPLETE",
...
for src, dst in (
        (OUT / cid / "B4_CELL_TERMINAL.json",
         EVID / "INTEGRATED_V5_TERMINAL_SAMPLE.json"),
        (OUT / cid / f"CLAIM_{b4a.CAMPAIGN_GENERATION}.json",
         EVID / "INTEGRATED_V5_CLAIM_SAMPLE.json"),
        (OUT / cid / f"SEAL_INTENT_{att}.json",
         EVID / "INTEGRATED_V5_SEAL_INTENT_SAMPLE.json"),
        (OUT / cid / f"SEAL_COMPLETE_{att}.json",
         EVID / "INTEGRATED_V5_SEAL_COMPLETE_SAMPLE.json")):
    dst.write_text(_san(src.read_text()))
(EVID / "INTEGRATED_V5_SUMMARY.json").write_text(json.dumps({
    "schema": "agent_multi.b4_integrated_v5_summary.v1",
