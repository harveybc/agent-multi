"""INTEGRATED V6C (order C39-C42): all 12 cells through the FULL\nrecovery-gated path under a fixture acta in a PRIVATE authority\nchain, with the REAL full-checkout identity (clean exact-commit\npositive) — no checkout stub. Original header follows:\n
...
OUT = STATE / "b4_integrated_v6cb_20260907"
...
_gate = Path.home() / ".cache/b4_iv6c_gate"
if _gate.exists():
    shutil.rmtree(_gate)
_am = _gate / "agent-multi"
_ra = _am / "reviewer_authority"
for _d in (_gate, _am, _ra):
    _d.mkdir(mode=0o700)
    os.chmod(_d, 0o700)
...
_ap = _ra / "MUSASHI_B4_V6_RECOVERY_AUDIT_RECORD.json"
_ap.write_text(json.dumps(_acta))
os.chmod(_ap, 0o600)
b4a.RECOVERY_AUDIT_RECORD_PATH = _ap
# NO checkout stub: the REAL full-checkout identity must pass —
# clean worktree at the exact pinned commit.
WITNESS = b4a.require_v6_launch_open()
print("fixture witness armed:", WITNESS["acta_sha256"][:12],
      "| pinned", WITNESS["pinned_commit"][:8],
      "| tree", WITNESS["checkout_tree_sha"][:8])
...
assert facts["n"] == 12
print(json.dumps({"integrated_v6c": "COMPLETE",
...
for src, dst in (
        (OUT / cid / "B4_CELL_TERMINAL.json",
         EVID / "INTEGRATED_V6C_TERMINAL_SAMPLE.json"),
        (OUT / cid / f"CLAIM_{b4a.CAMPAIGN_GENERATION}.json",
         EVID / "INTEGRATED_V6C_CLAIM_SAMPLE.json"),
        (OUT / cid / f"SEAL_INTENT_{att}.json",
         EVID / "INTEGRATED_V6C_SEAL_INTENT_SAMPLE.json"),
        (OUT / cid / f"SEAL_COMPLETE_{att}.json",
         EVID / "INTEGRATED_V6C_SEAL_COMPLETE_SAMPLE.json")):
    dst.write_text(_san(src.read_text()))
(EVID / "INTEGRATED_V6C_SUMMARY.json").write_text(json.dumps({
    "schema": "agent_multi.b4_integrated_v6c_summary.v1",
