...
- `../evidence/P1LR_CAUSAL_SCREEN_ACTIVITY_CROSSTAB_2026_08_16.json`
...
| AUD-F1-20260817-277 | S2 | correction required; partial `055ac32e` rejected as complete; active identity preserved | Thresholded SAC maps constant numerical bias to easy-mode directional activity and normal-mode HOLD, while the 1,000-epoch contract lacks a policy-responsiveness futility disposition | General Satoshi corrects 278-281 and completes WP2-WP5; General Musashi or Retsu independently verifies |
| AUD-F1-20260817-278 | S2 | correction independently reproduced at `4e2d3109`; awaits packet-level closure | Policy classifier drops malformed/non-finite actions and counts exact zero as a threshold-zero crossing | General Satoshi returns complete 277 packet; independent/owner authority closes |
| AUD-F1-20260817-279 | S2 | correction independently reproduced at `4e2d3109`; awaits packet-level closure | Policy sidecar bypasses role boundaries and lacks per-measurement trace/model/config custody binding | General Satoshi returns complete 277 packet; independent/owner authority closes |
| AUD-F1-20260817-280 | S3 | open; independently reproduced | Finding-277 delivery is not production-integrated, leaves an actor probe untracked and breaks the executable-surface gate | General Satoshi registers/commits tools, integrates four consumers and returns a green full suite |
| AUD-F1-20260817-281 | S2 | immediate correction independently reproduced at `4e2d3109`; observation-probe integration pending | Action variation is labeled state-responsive/promotable without any paired observation evidence | General Satoshi completes bound real-observation controls and four-consumer integration; independent/owner authority closes |
...
## Reconciliation note (append-only, 2026-08-10, Satoshi III)

Finding AUD-GEN-20260810-206: commit `939b6fac` was created from the
`fe6224aa` tree and integrated selected round-3 files without the
register delta carried by the audit branches. The originating audit,
Musashi reproducer source/result and correction order for findings
178-187 were restored byte-exact from commit `cc3f02ee`; the 188-200
register content and the appended 201-208 section come from the
