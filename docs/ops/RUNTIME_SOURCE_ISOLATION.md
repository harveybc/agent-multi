...
## Supervision code is NOT experiment source (finding AUD-GEN-20260816-261)

The pinned worktree isolates the EXPERIMENT source. A restart admission
gate is a different artifact class and must not live in the mutable
canonical checkout either: it decides whether a worker may start, so a
`git pull` would silently change admission. It also cannot be added to
the pinned worktree, which is immutable and predates it.

The lease gate is therefore installed as a separately versioned CONTROL
ARTIFACT: read-only, content-addressed by the sha256 of
`examples/systemd/p1lr-control/CONTROL_MANIFEST.sha256`, with that digest
written literally into the unit and re-verified (`sha256sum -c`, exit 4)
before every start. See `docs/ops/P1LR_V2_BOUNDARY_DEPLOY.md`.
