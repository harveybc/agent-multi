...
# --- the reward table: the one thing the closure table cannot show ------------------------------------------------------
#
# `evaluation/compare_stages.py` refuses the policy area by name, so every row of its table is NO_NEW_MEASUREMENT and
# the reward totals -- which are not a quality claim and were never offered as one -- do not appear in it. That is the
# right behaviour and it leaves a gap: a reader still has to be able to see the three arms' totals beside each other,
# on the rows the seal covers, without opening three JSON files and trusting that they are the same rows.
#
# So this renders exactly the owner's closure-table columns out of the same reports, and refuses the two columns it
# cannot fill. Skill is NOT_DEFINED because the reported quantity is a signed score and not an error: 1 - model/naive
# is not defined against a naive total of zero and would not mean skill if it were. What is shown instead is the plain
# arithmetic difference between two totals computed over identical sealed rows, named as a difference and not as an
# effect: one series in one simulation over one development sample is not a measurement of anything.

REWARD_TABLE_COLUMNS = ("stage", "status", "metric", "scale", "model value", "naive reference", "naive value",
                        "difference from naive", "skill", "literature value", "literature source", "comparability",
                        "rank")

SKILL_NOT_DEFINED = (
    "NOT_DEFINED: skill is 1 - model_error / naive_error on the same rows, and this quantity is a signed score, not "
    "an error; against a naive total of 0.0 the ratio is undefined and against any other it would not mean skill")

DECIMALS = 6

#: What one unit of the reported total is. Named rather than left implied, because a number with no unit is read as
#: whatever unit the reader already has in mind -- and here that would be money.
SCALE = ("the environment's own reward units under its configured reward plugin, summed over the replayed steps; "
         "they are units of a training signal and convert to nothing outside this simulation")


def reward_rows(reports):
    """One row per stage, read out of the reports' `environment_return` metric sets. Nothing here re-scores."""
    rows = []
    seals = {payload.get("corpus_seal") for payload in reports.values()}
    comparability = ("COMPARABLE" if len(seals) == 1 else
                     f"NOT_COMPARABLE: the stages carry {len(seals)} different corpus seals")
    for stage, payload in sorted(reports.items()):
        metrics = next((entry for entry in payload.get("metric_sets") or ()
                        if entry.get("name") == "environment_return"), None)
        if metrics is None:
            rows.append({"stage": stage, "status": "NO_NEW_MEASUREMENT", "value": None, "naive": None,
                         "difference": None, "comparability": "NO_NEW_MEASUREMENT",
                         "reason": "this report carries no environment_return metric set"})
            continue
        value = metrics["values"][REWARD_KEY]
        baseline = metrics.get("baseline")
        naive = float(baseline[REWARD_KEY]) if baseline else value
        rows.append({"stage": stage, "status": "MEASURED", "value": float(value), "naive": naive,
                     "difference": float(value) - naive, "comparability": comparability,
                     "scored_rows": metrics["counts"]["scored_rows"],
                     "is_naive": baseline is None, "reason": None})
    ranked = sorted((row for row in rows if row["status"] == "MEASURED" and row["comparability"] == "COMPARABLE"),
                    key=lambda row: (-row["value"], row["stage"]))
    position, previous = 0, object()
    for index, row in enumerate(ranked, start=1):
        if row["value"] != previous:                     # ties share a rank; nothing here breaks a tie by luck
            position, previous = index, row["value"]
        row["rank"] = position
    return rows


def reward_table_markdown(reports, *, seal=None, protocol=None):
    """The owner's closure-table columns for the reward, rendered sorted and with fixed decimals."""
    rows = reward_rows(reports)
    lines = ["## WP21(b) — the environment's own training reward, three arms, one seal", "",
             "Read this table with its first two sentences or not at all. "
             + REWARD_READING + ".", "",
             "`policy_profitability` stays refused by `m5phet_evaluation`, and the closure table generated by "
             "`evaluation/compare_stages.py` therefore carries NO_NEW_MEASUREMENT for every row of this area. This "
             "table exists to put the totals the reports do carry beside each other on the rows one seal covers; it "
             "is not a quality claim and no rank in it is evidence about any market.", ""]
    if seal is not None:
        lines += [f"Seal `{seal}` over {rows[0].get('scored_rows', '?')} scored rows"
                  + (f"; protocol `{protocol}`." if protocol else "."), ""]
    lines.append("| " + " | ".join(REWARD_TABLE_COLUMNS) + " |")
    lines.append("|" + "---|" * len(REWARD_TABLE_COLUMNS))
    for row in rows:
        if row["status"] != "MEASURED":
            lines.append("| " + " | ".join([row["stage"], row["status"], *(["NO_NEW_MEASUREMENT"] * 11)]) + " |")
            continue
        lines.append("| " + " | ".join([
            row["stage"], row["status"], REWARD_KEY, SCALE,
            f"{row['value']:.{DECIMALS}f}",
            "itself (this row IS the naive reference)" if row["is_naive"] else NAIVE_STAGE,
            f"{row['naive']:.{DECIMALS}f}",
            f"{row['difference']:.{DECIMALS}f}",
            "NOT_DEFINED",
            "NOT_CARRIED", "NOT_CARRIED",
            row["comparability"], str(row.get("rank", "NOT_RANKED"))]) + " |")
    lines += ["", "Skill column, in full: " + SKILL_NOT_DEFINED, "",
              "Literature column, in full: " + LITERATURE_NOT_CARRIED + "."]
    return "\n".join(lines) + "\n"
...
def main(argv=None):
    ...
    parser.add_argument("--markdown", default=None,
                        help="also write the reward table (the three arms' totals on the sealed rows) as Markdown")
    ...
    (out / "protocol.json").write_text(json.dumps(built["protocol"].as_dict(), indent=2, sort_keys=True) + "\n",
                                       encoding="utf-8")
    table = reward_table_markdown(built["reports"], seal=built["seal"].seal, protocol=built["protocol"].digest)
    (out / "reward_table.md").write_text(table, encoding="utf-8")
    if args.markdown:
        Path(args.markdown).write_text(table, encoding="utf-8")
    print(json.dumps({"reports": written, "reward_table": str(out / "reward_table.md"),
                      "corpus_seal": built["seal"].seal,
