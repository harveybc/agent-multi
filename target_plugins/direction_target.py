"""Direction target plugin for direction classification predictors.

Maps a single direction label column to the y_train / y_val / y_test dict
format expected by the pipeline.  The ``signal_type`` parameter selects
which label column to extract.
...
SIGNAL_MAP = {
    "direction_long":  ("direction_long_label",  "direction_long_binary"),
    "direction_short": ("direction_short_label", "direction_short_binary"),
}
...
class TargetPlugin:
    ...
    def calculate_targets_from_baselines(self, baseline_data, config):
        """Extract direction labels aligned to sliding-window count.
        ...
        for split in ("train", "val", "test"):
            ...
            print(
                f"[direction_target] {split} {output_name}: "
