...
SSH_OPTS = ["-o", "BatchMode=yes", "-o", "ConnectTimeout=10", "-p", "22022"]
POLL_SEC = 120
DONE_GRACE_SEC = 90
...
HOSTS: dict[str, dict[str, Any]] = {
    "dragon": {
        "addr": "harveybc@192.0.2.11",
        "kind": "remote",
        "max_concurrent": 2,
    },
    "gamma": {
        "addr": "harveybc@192.0.2.12",
        "kind": "remote",
        "max_concurrent": 1,
    },
    "omega": {
...
def sync_back(task: dict[str, Any]) -> None:
    ...
    cmd = ["rsync", "-az", "-e", "ssh -p 22022 -o BatchMode=yes", remote, f"{run_dir}/"]
